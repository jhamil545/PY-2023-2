...
import 'package:asistencia_app/ui/Especie/MyAppState.dart';
...
import 'package:asistencia_app/ui/especie/especie_edit.dart';
import 'package:asistencia_app/ui/especie/especie_form.dart';
...
class MainEspecie extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => EspecieBloc(EspecieRepository())),
      ],
      child: MaterialApp(
...
class EspecieUI extends StatefulWidget {
  @override
  _EspecieUIState createState() => _EspecieUIState();
}

class _EspecieUIState extends State<EspecieUI> {
  //ApiCovid apiService;
  ...
  void initState() {
    ...
    BlocProvider.of<EspecieBloc>(context).add(ListarEspecieEvent());
    print("entro aqui");
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => EspecieForm()),
                  ).then(onGoBack);
                },
        ...
        body: BlocBuilder<EspecieBloc, EspecieState>(
          builder: (context, state) {
            if (state is EspecieLoadedState) {
              return _buildListView(context, state.EspecieList);
            } else {
              return Center(
        ...
        /*body: FutureBuilder<List<EspecieModelo>>(
          future: Provider.of<EspecieApi>(context, listen: true)
              .getEspecie(TokenUtil.TOKEN).then((value) => value),
          builder: (BuildContext context,
              AsyncSnapshot<List<EspecieModelo>> snapshot) {
            if (snapshot.hasError) {
              ...
              List<EspecieModelo> persona = snapshot.data!!;
  ...
  Widget _buildListView(BuildContext context, List<EspecieModelo> persona) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),
      child: ListView.builder(
        itemBuilder: (context, index) {
          EspecieModelo personax = persona[index];
          return Padding(
                                            ...
                                            onPressed: () {
                                              showDialog(
                                                ...
                                                if (value.toString() ==
                                                    "Success") {
                                                  print(personax.id);
                                                  /*Provider.of<EspecieApi>(context,
                                                  listen: false)
                                                  .deleteEspecie(TokenUtil.TOKEN,personax.id)
                                                  .then((value) => onGoBack(value));*/
                                                  //var onGoBack = onGoBack;
                                                  BlocProvider.of<EspecieBloc>(
                                                          context)
                                                      .add(DeleteEspecieEvent(
                                                          personax));
                                                }
                                              });
                                            }))
  ...
  void exportAsistenciaToExcel(List<AsistenciapaxModelo> asistencia) {
    ...
    for (var row = 0; row < asistencia.length; row++) {
      ...
      sheetObject
          ...
          .value = asistenciax.Especie.nombreEspecie;
      sheetObject
