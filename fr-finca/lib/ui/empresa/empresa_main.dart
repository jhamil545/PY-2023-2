...
import 'package:asistencia_app/ui/empresa/MyAppState.dart';
...
import 'package:asistencia_app/ui/empresa/empresa_edit.dart';
import 'package:asistencia_app/ui/empresa/empresa_form.dart';
...
class MainEmpresa extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    /*return Provider<EmpresaApi>(
      create: (_) => EmpresaApi.create(),
      child: MaterialApp(
...
class _EmpresaUIState extends State<EmpresaUI> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        /*body: FutureBuilder<List<EmpresaModelo>>(
          future: Provider.of<EmpresaApi>(context, listen: true)
              .getEmpresa(TokenUtil.TOKEN).then((value) => value),
          builder: (BuildContext context,
              AsyncSnapshot<List<EmpresaModelo>> snapshot) {
            if (snapshot.hasError) {
              ...
              List<EmpresaModelo> persona = snapshot.data!!;
  ...
  Widget _buildListView(BuildContext context, List<EmpresaModelo> persona) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),
      child: ListView.builder(
        itemBuilder: (context, index) {
          EmpresaModelo personax = persona[index];
          return Padding(
                                    ...
                                    child: IconButton(
                                      ...
                                      onPressed: () {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => EmpresaFormEdit(modelA: personax),
                                          ),
                                        ).then(onGoBack);
                                      },
                                    ),
                                      ...
                                      onPressed: () {
                                        showDialog(
                                          ...
                                          if (value.toString() == "Success") {
                                            print(personax.id);
                                            BlocProvider.of<EmpresaBloc>(context).add(
                                              DeleteEmpresaEvent(personax),
                                            );
                                          }
                                        });
                                      },
